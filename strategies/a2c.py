"""A2C (Advantage Actor-Critic) pour l'exploration"""
...
class A2CStrategy:
    """Stratégie A2C pour l'exploration"""
    def __init__(self, state_size=4, action_size=4):
        self.state_size = state_size
        self.action_size = action_size
        
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.network = A2CNetwork(state_size, action_size, Config.PPO_HIDDEN_SIZE).to(self.device)
    ...
    def decay_epsilon(self):
        """A2C n'utilise pas epsilon-greedy"""
        pass
    
    def save_policy(self, filename='a2c_model.pth'):
        """Sauvegarde le modèle"""
        torch.save({
            'network': self.network.state_dict(),
            'optimizer': self.optimizer.state_dict()
        }, filename)
        print(f"Modele A2C sauvegarde dans {filename}")
    
    def load_policy(self, filename='a2c_model.pth'):
        """Charge un modèle"""
        try:
            checkpoint = torch.load(filename, map_location=self.device)
            self.network.load_state_dict(checkpoint['network'])
            self.optimizer.load_state_dict(checkpoint['optimizer'])
            print(f"Modele A2C charge depuis {filename}")
            return True
